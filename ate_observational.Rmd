---
title: "Average Treatment Effect on the treated with Propensity Score"
output:
  html_document:
    df_print: paged
---
...
## 1. Estimate the propensity score using logistic model for PSID-1 and CPS-1
...
We discard controls with propensity score less than the minimum propensity score of treated group. We use the logistic model provided by Dehejia and Wahba to estimate the propensity score[^2].

[^2]: Dehejia, R. and Wahba, S. (1998) *Propensity score matching methods for non-experimental causal studies*. <doi:10.3386/w6829>.
...
pscore_psid1 <- estimate_propensity_score(psid1_re74, 
                ...
                u74 * black, is_plot=TRUE)
```

## 2. Matching and Estimate Average Treatment Effect (ATE)

#### i. PSID1

Following Dehejia and Wahba (1998), I will try to do 1-to-1 nearest propensity score matching and with replacement.
...
As we can see, the treated units were well matched across all propensity scores. We can also reproduce the average treatment effect on the treated of Match() using these estimators[^3]:
...
For bias adjustment estimator proposed by Abadie, Alberto and Guido Imbens[^4],

[^4]: Abadie, Alberto and Guido Imbens. 2006. “Large Sample Properties of Matching Estimators for Average Treatment Effects.” *Econometrica* 74(1): 235-267.
...
-   As X can have high dimensions, propensity score matching estimators (Rosenbaum and Rubin (1983)) are widely used to estimate treatment effects.[^5] When the relevant differences between any two units are captured in the observed covariates, matching methods can yield an unbiased estimate of the treatment impact.[^6] Therefore, propensity score matching aims to balance the covariate differences between treated and control groups.

-   As we can never observe Yi(1) and Yi(0) together, we can use matching to impute the missing potential outcomes.[^7]
...
-   How can I improve the result
    ...
    -   Assessing the quality of the resulting matched samples, and perhaps iterating with steps 1 and 2 until well-matched samples result.[^8]
...
[^7]: Abedie and Imbens (2006)

[^8]: Elizabeth A. Stuart, Matching Methods for Causal Inference: A Review and a Look Forward
...
We have this inverse propensity score weighting estimator[^9].

[^9]: Ding, Peng. (2023). A First Course in Causal Inference. 10.48550/arXiv.2305.18793.
...
```{r}
# IPW ATE
ate.psid1.ipw <- ps_IPW(filtered_psid1, estimand="ATE", norm=FALSE)
paste("IPW ATE: ", ate.psid1.ipw$est)
```

```{r}
# IPW ATE
att.psid1.ipw <- ps_IPW(filtered_psid1, estimand="ATT", norm=FALSE)
...
Some weights are very large, which may violate the overlap condition. What if we do more trimming?

```{r}
rm.idx <- which(filtered_psid1$ps < 0.1 | filtered_psid1$ps > 0.9)

# PS distribution
plot_pscore(filtered_psid1[-rm.idx, ], filtered_psid1$ps[-rm.idx])
```

```{r}
trim_out1_IPW_ATE <- ps_IPW(filtered_psid1[-rm.idx, ], estimand="ATE")
paste("IPW ATE: ", trim_out1_IPW_ATE$est)
```

What if we do not discard any controls,

```{r}
# Compute propensity scores but do not discard any controls
no_trim_psid1_re74 <- preprocess_data(psid1_re74)

formula <- treat ~ age + I(age^2) + education + I(education^2) +
  married + nodegree + black + hispanic + re74 + I(re74^2) +
  re75 + I(re75^2) + u74 * black

ps_model <- glm(formula, family = binomial, data = no_trim_psid1_re74)
```

```{r}
# Compute IPW ATE
no_trim_psid1_re74$ps <- ps_model$fitted.values
paste("IPW ATE: ", ps_IPW(no_trim_psid1_re74, "ATE")$est)
```

```{r}
weights <- ps_IPW(no_trim_psid1_re74, "ATE")$weights
treated_weights <- weights[no_trim_psid1_re74$treat == 1]
control_weights <- weights[no_trim_psid1_re74$treat == 0]

# Create side-by-side boxplots
par(mfrow = c(1, 2))  # Set up the plotting area for 1 row and 2 columns
...
It seems like trimming significantly improves the estimate as we do not violate the strong overlap assumption, which is the propensity score is bounded away from 0 and 1.
...
# Create a list of datasets after truncating the estimated propensity score
trim_datasets <- lapply(datasets, function(dataset) {
  rm.idx <- which(dataset$ps < 0.1 | dataset$ps > 0.9)
  dataset <- dataset[-rm.idx, ]
  return(dataset)
} )
```
...
out_list <- lapply(datasets, function(data) {
...
row_names <- c("NSW", "PSID-1", "PSID-2", "PSID-3", "CPS-1", "CPS-2", "CPS-3")
col_names <- c("IPW ATE")
(ate.ipw.results <- create_psm_result(out_list, row_names, col_names))
```

```{r}
# Compute ATE using IPW for trimmed datasets
out_list <- lapply(trim_datasets, function(data) {
  ps_IPW(data, estimand = "ATE")
})
...
row_names <- c("NSW", "PSID-1", "PSID-2", "PSID-3", "CPS-1", "CPS-2", "CPS-3")
col_names <- c("IPW ATE")
(trim_ate.ipw.results <- create_psm_result(out_list, row_names, col_names))
```
...
row_names <- c("NSW", "PSID-1", "PSID-2", "PSID-3", "CPS-1", "CPS-2", "CPS-3")
col_names <- c("IPW ATT")
(att.ipw.results <- create_psm_result(out_list, row_names, col_names))
```
...
Under strong ignorability and overlap condition. As the formulas can be very big if the propensity score is very close to 1 or 0, the weighting estimators blow up to infinity resulting in extremely unstable behaviors in finite samples.[^10] As a result, the more we trimmed the psid1 dataset, the better estimate we got.

[^10]: Peng Ding

-   Treated subjects with a very low propensity score can result in a very large weight. Similarly, a control subject with a propensity score close to one can result in a very large weight. Such weights can increase the variability of the estimated treatment effect.[^11] (Why?)

[^11]: P. C. AUSTIN AND E. A. STUART

### i. ATE

```{r}
cbind(ate.result_table, ate.ipw.results)
```

### ii. ATT

```{r}
cbind(att.result_table, att.ipw.results)
```
